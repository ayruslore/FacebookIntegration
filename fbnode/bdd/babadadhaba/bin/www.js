...
let token = 'EAADg7MiZAzmsBAECP308XVFSKNfDB51cfAgalhRUxVVZAR5fEyyn81eqsD4PkcJlyq9MEfNhk1pUWKgc3v1HaJkOZC4apvtHpt30Mm4Ahz3xJ5ifjB7HZBfkniu8GMkwe31pd9TN0Awg52G5BiNgRIV3GWsDcIqmOeHdsph4uQZDZD';
...
app.get('/confirm', function (req, res) {
  ...
  //console.log(req);
       ...
       // var operation = activelink + paymentId +"/get_payment_status_2";
       sendTextMessage(userId,"Your Order has been placed.");
       var operation =activelink + "get_receipt/" +userId ;
...
function sendMessage(event){
        ...
        apiai.on('response', (response) => {
                ...
                if(response.result.action==='recommend.specials'){
                        ...
                        sendButton(sender,["web_url"],'Make the payment',["http://ec2-13-58-254-247.us-east-2.compute.amazonaws.com/payment.html?userid=" + sender],["CASH ON DELIEVERY"],"tall");
                }
...
function receivedPostback(event) {
        ...
        if (payload) {
                ...
                switch (payload) {
                        ...
                        break;
                        case 'Confirm_Yes':
                                var link =activelink + "is_carts/" +senderID;
                                callSendRedisapi("iscart",link,senderID,"Please choose your delivery address");
                                //CustomQuickreply(senderID,"Please choose your delivery address");

                        break;
                        case 'Confirm_bypass':
                                var operation= activelink + 'bypass_payments/' + senderID;
                                callSendRedisapi('receipt',operation,senderID,aiText)
                        break;
                        case 'GET_STARTED_PAYLOAD':
                                console.log("greeting2");
                                var link = "https://graph.facebook.com/v2.6/"+senderID+"?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token="+token;
                                callSendRedisapi('greeting',link,senderID,aiText);
                        break;
                        case 'Show_specials':
                                var final= recommendationLink + 'specials';
                                callrecommend(final,senderID);
                        break;
                        case 'Recommend':
                                callrecommend(recommendationLink+ senderID + '/get_history_reco',senderID);
                        break;
                        case 'Confirm_order':
                               /* aiText="Do you want to confirm this order?";
                                sendTextMessage(senderID ,aiText);*/
                                var show = 'get_cart_price/'+senderID;
                                var final= activelink + show;
                                callSendRedisconfirm(final,senderID);
                        break;
                        case 'Clear_cart':
                                var cancel='cart/' + senderID + '/cancel';
                                final =activelink + cancel;
                                aiText='Your cart has been cleared. Is there anything else I can help you with?';
                                callSendRedisapi('cancel',final,senderID,aiText);
                        break;
                        case 'Show_cart':
                                var show = 'get_cart_price/'+senderID;
                                var final= activelink + show;
                                callSendRedis(final,senderID);
                        break;
                        default:
                                console.log(payload);
...
function sendButton(recipientId,type,text,payload,caption,size){
        ...
        console.log("length of array is:"+type.length);
...
function callSendRedisapi(tag,operation,sender,aiText){
        request_1.getAsync({
                ...
                if (tag==='add'){
                        ...
                        link= "http://ec2-13-58-254-247.us-east-2.compute.amazonaws.com/payment.html?userid"+sender;
                        ...
                        callSendRedisapi("setname",link,sender," ");
                }
                else if(tag === 'setname'){
                        console.log("res.body");
                        var response=JSON.parse(res.body);
                        sendTextMessage(sender,"Hi "+ response.first_name +"!! It's nice to see you! Welcome to Baba da Dhaba! I am transformer Sardar! I custom tailor your food suggestions based on your past choices. The more you order from me, the better I get at my job.");
                        var aiText= "Hi "+ response.first_name +"!! It's nice to see you! Welcome to Baba da Dhaba! I am transformer Sardar! I custom tailor your food suggestions based on your past choices. The more you order from me, the better I get at my job.";
                        var link="http://ec2-13-58-254-247.us-east-2.compute.amazonaws.com?userId=" + sender;
                        sendButton(sender,["postback","postback","web_url"],aiText,["Show_specials","Recommend",link],["Specials", "Recommendations", "Menu"],"tall");
                }
                        ...
                        else if (response.usual==="Nothing")
                        {
                                ...
                                if ((time>=1)&&(time<2)){
                                        ...
                                        message= "Hi "+ name +". It's nice to see you back in 24 hours. You clearly loved out food. Here are your usuals! Be sure to tell me if I can help you out with anything else!!";
                                }
                                else {
                        ...
                        sendQuickreply(sender,"Your order is "+res.body,["Check Status"],["order_status"]);
                }
                else if (tag==='receipt'){
                        var keys =[];
                        var quantity=[];
                        ...
                        sendButton(sender,["postback"],"Your order status is: "+toTitleCase(response.order_status),["order_status"],["Check Status"]);
                        console.log(response.adjustments);
                      }
                      else{
...
function callSendRedis(operation,sender){
        ...
        request_1.getAsync({
                ...
                if ((res.body==='{}') || (JSON.parse(res.body).total===0))
                {
                        ...
                        var link="http://ec2-13-58-254-247.us-east-2.compute.amazonaws.com?userId=" + sender;
                        sendButton(sender,["web_url","postback","postback"],c,[link,"Clear_cart","Confirm_order"],["Edit cart", "Clear cart", "Confirm order"],"tall");
                }
        });
}
...
function callSendRedisAddress(operation,sender){
        request_1.getAsync({
                ...
                var body = ''; // Will contain the final response
                if(err) throw err;
                var link= "http://ec2-13-58-254-247.us-east-2.compute.amazonaws.com/payment.html?userid="+sender;
                sendButton(sender,["web_url"],'how would you like to pay?',[link],["Payments"],"tall");
        });
}
